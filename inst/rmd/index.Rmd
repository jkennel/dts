...
# smooth for calibration - very minor improvement
y <- dts$trace_time$calib_temperature
x <- 1:nrow(dts$trace_time)
fit <- lm(y~splines::ns(x, df = 30))$fitted.values
dts$trace_time$calib_temperature <- fit
...
# calculate run length encoding on sign of change
# if the length is short and the total change is small move towards the mean
...
for(i in seq_along(1:1500)) {
  tmp <- rle(sign(diff(dts$trace_data[distance == non_bath_distance[i]]$temperature)))
  print(table(tmp$lengths))
  wh <- which(tmp$lengths == max(tmp$lengths))
  print(wh)
  print(cumsum(tmp$lengths)[wh])
  id <- c(id, cumsum(tmp$lengths)[wh])
}
plot(sort(id), type = 'l')

plot(dts$trace_data[distance == non_bath_distance[600]]$temperature[600:800], type = 'l')
abline(v = as.numeric(names(head(rev(sort(table(id))), 15)))-600, lty = 2,lwd = 3, col = 'red')

plot(temperature~calib_temperature, bath_dts[distance == bath_distance[5]][1:100], pch = 20, cex = 0.6, type='l')
points(temperature~calib_temperature, bath_dts[distance == bath_distance[6]][1:100], pch = 20, cex = 0.6, col = 'red', type = 'l')


grp <- bath_dts[, list(mean_dts_temp = mean(temperature),
                mean_calib_temp = mean(calib_temperature)), by = start]

m <- to_matrix(bath_dts[distance < 100], col_name = 'temperature')
...
a <- to_matrix(dts)
sub <- 1:300
y_lab <- as.numeric(rownames(a))

x_lab <- as.POSIXct(as.numeric(colnames(a)[sub]),
...
x <- 1:length(q)
plot(o[1:100], type = 'l')
points(p[1:100], type = 'l', col = 'red')
points(p - (lm(q~splines::ns(x, 100))$residuals+lm(r~splines::ns(x, 100))$residuals)/2, col = 'blue', type = 'l')
mean(abs(diff(a[101, ])))
plot_ly(z = a[,sub],
        # x = x_lab,
        # y = y_lab,
...
# par(new = TRUE)
# plot(dts::subset_distance(dts, begin = 90, end = 90.1)$trace_data$temperature, col = 'red', type = 'l')
