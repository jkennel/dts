...
#'
#' @examples
write_dts_to_wellcad <- function(x, output_dir, ...) UseMethod("write_dts_to_wellcad")



#' @rdname write_dts_to_wellcad
#' @export
write_dts_to_wellcad.dts_long <- function(x, output_dir = getwd(), max_columns = 1000, ...) {
  ...
  for (i in subs) {
    ...
    fn <- file.path(output_dir, paste0('dts_data_', i, '_', end_col, '.csv'))
    out <- cbind(distance = get_distance_table(x)$distance,
